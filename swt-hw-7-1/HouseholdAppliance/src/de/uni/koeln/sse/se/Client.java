...
public static void main(String[] args) {
	...
	PackagingInstructionAssigner packagingInstructionAssigner = new PackagingInstructionAssigner();
		...
		for (int i = 0; i < householdItems.size(); i++) {
			System.out.println("Total Cost for " + householdItems.get(i).getName() + " is: " + priceCalculator.getPrice(householdItems.get(i)) + " Euros");
		}

	for (int i = 0; i < householdItems.size(); i++) {
		System.out.println(householdItems.get(i).getName() + ":\n " + packagingInstructionAssigner.getInstructions(householdItems.get(i)));
	}
